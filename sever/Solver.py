...
class Solver:
    ...
    def negamax(self, P: Position, alpha: int, beta: int) -> int:
        ...
        for i in reversed(range(Position.WIDTH)):
            move = possible & Position._column_mask(self.columnOrder[i], Position.HEIGHT)
            if move:
        ...
        while True:
            ...
            if not next_move:
                break
            # Sử dụng push_state và pop_state thay vì copy
            P.push_state()            # Lưu trạng thái hiện tại của P
            P.play(next_move)         # Thực hiện nước đi trên P
            score = -self.negamax(P, -beta, -alpha)
            P.pop_state()             # Khôi phục lại trạng thái đã lưu
    ...
    def solve(self, P: Position, weak: bool = False) -> int:
        ...
        while min_score < max_score:
            med = (min_score + max_score) // 2
            if med <= 0 and min_score < med:
                med = (min_score + max_score + 1) // 2
            else:
                med = (min_score + max_score) // 2
            
            # Gọi negamax với cửa sập [med, med+1]
